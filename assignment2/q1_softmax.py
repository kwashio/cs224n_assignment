...
def softmax(x):
    ...
    ### YOUR CODE HERE
    # 行ごとに最大値を計算、expand_dimsでshapeを(xの行, 1)に
    row_max = tf.expand_dims(tf.reduce_max(x, axis=1), 1)
    ...
    # 行ごとに総和をとり、softmaxの分母を計算
    denominator = tf.expand_dims(tf.reduce_sum(exp, axis=1), 1)
